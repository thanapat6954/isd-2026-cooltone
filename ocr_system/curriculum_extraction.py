...
if __name__ == "__main__":
    ...
    ocr_file = sys.argv[1] if len(sys.argv) > 1 else "outputs/sample_ocr.json"
    
    print(f"extracting from: {ocr_file} ...")
    ...
    try:
        ...
        print("\n"
        "extract result (Extracted Curriculum) ---")
        print(json.dumps(result, indent=2, ensure_ascii=False))
        
        output_path = Path(ocr_file).parent / "extracted_curriculum.json"
        with open(output_path, "w", encoding="utf-8") as f:
            json.dump(result, f, indent=2, ensure_ascii=False)
        print(f"\n:successfully extract. output at {output_path}")
        
    except FileNotFoundError:
        print(f"do not found orc file: {ocr_file}")